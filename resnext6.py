...
def load_autoencoder(filters = 64):
	encoder_file = os.path.join(os.getcwd(), 'imagenet-encoder2-weights', 'imagenet-encoder2-imagenet.pth')
	decoder_file = os.path.join(os.getcwd(), 'imagenet-decoder2-weights', 'imagenet-decoder2-imagenet.pth')
	encoder, decoder = create_autoencoder(filters = filters, imagenet = True)
...
class ResNext(nn.Module):
	def __init__(self, datashape, nclasses, enc, nblocks = [3, 4, 6, 3], infilters = 64, planes = 64, expansion = 4, cardinality = 32, width = 4, base = 64, relu = True):
...
if __name__ == '__main__':
	...
	print('resnext experiment from resnext6.py with parameters')
	...
	encoder, decoder = load_autoencoder(args.filters)
	test_autoencoder(datashape, encoder, decoder, testloader, mean, std)
	
	model = ResNext(datashape, nclasses, encoder, relu = args.relu)
