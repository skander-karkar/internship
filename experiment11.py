...
def diagImpConvFFT(x, B, h, mode = None): # convolution using FFT of (I + h B' B)^-1
	n = x.shape
	m = B.shape
	mid1 = (m[2] - 1) // 2
	mid2 = (m[3] - 1) // 2
...
class FirstResBlock(nn.Module):
	def __init__(self, filters, batchnorm, bias, h):
		super(FirstResBlock, self).__init__()
		self.h = h
		self.B = nn.Parameter(torch.rand(filters, 1, 3, 3))
	...
	def forward(self, x):
		...
		z = self.cv2(z)
		q = convDiag(x, self.B)
		q = convDiagT(q, self.B) 
		x = x + self.h * z + self.h * q
		# x = self.bn3(x)
		x = diagImpConvFFT(x, self.B, self.h, mode = 'rand')
		return x, z 
...
class OneRepResNet(nn.Module):
	def __init__(self, datashape, nclasses, filters, nblocks, batchnorm, bias, h, encoder, initname = 'orthogonal', initgain = 0.01,
...
def test(model, criterion, optimizer, testloader, decoder, mean, std, lambdatransport, lambdaloss, e, folder, dist = True, tsne = False):
	...
	for j, (x, y) in enumerate(testloader):
		...
		loss.backward()
		_, pred = torch.max(out.data, 1)
		update_meters(y = y, pred = pred, loss = loss.item(), loss_meter = loss_meter, acc_meter = acc_meter, trs = transport.item(), trs_meter = trs_meter)
		if j == idx_batch:
			idx_images = np.random.choice(x.size()[0], 5, replace = False)
...
def experiment(dataset_name, filters, learningrate, lambdatransport, lambdaloss0, tau, uzawasteps, batchnorm, bias, h, clip, learnclassifier, nblocks, 
	...
	print('--- experiment from experiment11.py with parameters')
	for name in names:
	...
	model = OneRepResNet(datashape, nclasses, filters, nblocks, batchnorm, bias, h, encoder, initname, initgain, learnclassifier, classifier_name)
	model.to(device)
	print(model)
...
if __name__ == '__main__':
	...
	parser.add_argument("-hhh", "--hstep", type = float, default = [1], nargs = '*')
